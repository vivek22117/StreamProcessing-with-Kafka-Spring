...
data "template_file" "zookeeper" {
  ...
  vars {
    volume_id_1 = "${element(data.terraform_remote_state.kafka_fixed_resource.zk_ec2_volumes, 0)}"
    volume_id_2 = "${element(data.terraform_remote_state.kafka_fixed_resource.zk_ec2_volumes, 1)}"
    volume_id_3 = "${element(data.terraform_remote_state.kafka_fixed_resource.zk_ec2_volumes, 2)}"

    eni_id_1 = "${element(data.terraform_remote_state.kafka_fixed_resource.zk_eni_ids, 0)}"
    eni_id_2 = "${element(data.terraform_remote_state.kafka_fixed_resource.zk_eni_ids, 1)}"
    eni_id_3 = "${element(data.terraform_remote_state.kafka_fixed_resource.zk_eni_ids, 2)}"

    zk_ip_1 = "${element(data.terraform_remote_state.kafka_fixed_resource.zk_eni_ips, 0)}"
